<?php
// admin/module/fasilitas/upload.php

// Naik 3 tingkat dari admin/module/fasilitas/ ke root admin/
$serverAdminBase = dirname(__DIR__, 3) . '/'; 
// Target folder Thumbnail Fasilitas
$thumbDir = $serverAdminBase . 'public/uploads/thumb/fasilitas-thumb/';
...
function handleUpload($fieldName, $oldFile, $uploadDir, $allowedExt, $maxSize, $newSlug) {
    ...
    // --- KASUS 1: TIDAK ADA FILE BARU (RENAME SLUG) ---
    ...
    // --- KASUS 2: FILE BARU DI-UPLOAD ---
    $file = $_FILES[$fieldName];

    if ($file['error'] !== UPLOAD_ERR_OK) {
        throw new Exception("Gagal upload. Kode: " . $file['error']);
    }
    ...
    if (!in_array($ext, $allowedExt)) {
        @unlink($file['tmp_name']); 
        throw new Exception("Ekstensi tidak diizinkan.");
    }
    
    // Hapus file lama & thumbnail lama
    if ($oldFile) {
        $old_thumb_name = pathinfo($oldFile, PATHINFO_FILENAME) . '-thumb.' . pathinfo($oldFile, PATHINFO_EXTENSION);
        if (is_file($thumbDir . $old_thumb_name)) {
            @unlink($thumbDir . $old_thumb_name);
        }
    }
...
// Fungsi createStaticThumbnail SAMA PERSIS dengan produk/upload.php
function createStaticThumbnail(string $sourcePath, string $targetPath, int $targetWidth = 300, int $quality = 90): bool
{
    ...
    try {
        ...
        if ($ext === 'png') imagepng($tmp, $targetPath, 9);
        else imagejpeg($tmp, $targetPath, $quality);
